...
export class GameView {
  constructor() {
    // Riferimenti ai principali elementi HTML (ex kb.js)
    // Overlay Info
  ...
  // =============== BINDING EVENTI ===============
  /**
   * Riceve un oggetto di callback su come gestire i vari pulsanti 
   * (startRound, nextRound, showSolution, hideSolution, mainMenu...) 
   */
  bindUIEvents(callbacks) {
    ...
    if (this.hideSolutionButton) {
      this.hideSolutionButton.addEventListener("click", () => {
        if (callbacks.onHideSolution) callbacks.onHideSolution();
      });
    }
    if (this.playSolutionButton) {
      this.playSolutionButton.addEventListener("click", () => {
    ...
    if (this.assistantModeButton) {
      this.assistantModeButton.addEventListener("click", () => {
  ...
  bindGlobalListeners(model, callbacks) {
    document.addEventListener("keydown", (event) => {
      ...
      if (!model.isPracticeMode) {
        callbacks.onCheckChord();
      } 
      else {
        callbacks.onIdentifyChord();
      }
    });
    ...
    document.addEventListener("mousedown", (event) => {
      ...
      if (callbacks.getIsSelectedChords() && callbacks.getIsAssistantModeOn()) {
        if (callbacks.getChordData().midiNotes.includes(midiNote)) {
          callbacks.colorKey(midiNote, "green", true);
        } else {
          callbacks.colorKey(midiNote, "red", true);
        }
      }
      else if (callbacks.getIsSelectedHarmony() && callbacks.getIsAssistantModeOn()) {
        if (callbacks.getCadenceData().missingChordData.midiNotes.includes(midiNote)) {
          callbacks.colorKey(midiNote, "green", true);
        } else {
          callbacks.colorKey(midiNote, "red", true);
        }
    ...
    switch(overlayType) {
      case "startGame":
        ...
        if (model.selectedGameMode === "chords_GM") {
          this.overlayTitle.innerHTML = "RECOGNIZE CHORD AND PLAY IT";
        } else {
          this.overlayTitle.innerHTML = "RESOLVE CHORD CADENCES PLAYING MUTED LAST CHORD";
        }
      ...
      case "showSolution":
        ...
        if (model.selectedGameMode === "chords_GM") {
          this.overlayTitleSolution.innerHTML = 
            `IT'S A ${model.generatedChordData.noteRoot}${model.generatedChordData.chordType} ` +
            `IN ${model.generatedChordData.inversion}`;
          this.overlaySubtitleSolution.innerHTML = "";
        } else if (model.selectedGameMode === "harmony_GM") {
          const missing = model.generatedCadenceData.missingChordData;
          this.overlayTitleSolution.innerHTML = 
            `IT'S A ${model.generatedCadenceData.name} IN ` + 
            `${missing.noteRoot}${missing.chordType}`;
          this.overlaySubtitleSolution.innerHTML = 
            `${model.generatedCadenceData.cadenceName} - ` + 
            `${missing.noteRoot}${missing.chordType}`;
        }
        break;
  ...
  hidePlacement() {
    if (this.placementDisplay) {
      this.placementDisplay.style.display = "none";
    }
  }
}
